...
ws.on('open', () => {
  const jsCode = `
    (function() {
      // Find and click the Teams step
      const elements = document.querySelectorAll('[class*="card"], [class*="operation"], button, [role="button"]');
      for (const el of elements) {
        const text = el.textContent || '';
        if (text.includes('Post message in a chat or channel')) {
          el.click();
          return 'Clicked Teams step';
        }
      }
      return 'Teams step not found';
    })();
  `;
